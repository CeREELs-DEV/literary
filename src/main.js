...
// one button opens the player with three PRE-PRODUCED videos — the same
// paragraph through Felicity's eyes, Jonah's eyes, and the world's. No
// prompts, no models, no generation UI ever reaches this screen.
import { PASSAGE, SCENES, COMPARE_QUESTIONS } from './player-data.js'
...
function renderPlayer() {
  ...
  for (const scene of SCENES) {
    ...
    tab.style.setProperty('--accent', scene.color)
    tab.innerHTML = `<span class="vp-icon">${scene.icon}</span>${scene.title}`
    tab.addEventListener('click', () => {
      activeSceneId = scene.id
      renderPlayer()
    })
    viewpointTabs.appendChild(tab)
  }
  ...
  video.src = scene.videoAssetUrl
  if (scene.thumbnailUrl) video.poster = scene.thumbnailUrl
  video.muted = true
  video.loop = true
  video.autoplay = true
  ...
  video.addEventListener('error', () => {
    ...
    placeholder.style.setProperty('--accent', scene.color)
    placeholder.innerHTML =
      `<span class="vp-icon">${scene.icon}</span>` +
  ...
  sceneInterpretation.textContent = `“${scene.interpretation}”`
  sceneInterpretation.style.setProperty('--accent', scene.color)
