...
- `server/images.js` — 레퍼런스 로드 + 비트별 이미지 생성(`loadReferenceImages`, `generateBeatImages`)
...
- `public/images/*.png` — 사용자 제공 그림체 레퍼런스 4장 (이미 배치됨 — 오즈 캐릭터 스프라이트; 전부 레퍼런스로 사용)
- `public/audio/gomtang s3.mp3` — 사용자 제공 배경음악 (이미 배치됨)
...
## Task 7: 에셋 확인 + E2E 수동 검증 (키·에셋 필요)

**Files:** 없음 (에셋은 이미 배치됨)

- [ ] **Step 1: 사용자 에셋 배치 확인** (이미 완료된 상태여야 함)

```bash
ls public/images/*.png "public/audio/gomtang s3.mp3" && grep -c GEMINI_API_KEY .env
```
Expected: 레퍼런스 PNG 4장 + mp3 존재, GEMINI_API_KEY 설정됨. 에셋을 커밋할지는 사용자 판단(용량 작으니 커밋 권장):

```bash
git add public && git commit -m "chore: add reference images and bgm assets"
```

- [ ] **Step 3: 시각 단계 스킵 경로 확인 (GEMINI_API_KEY를 잠시 주석 처리하고)**
